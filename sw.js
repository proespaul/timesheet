 ...
 */
const CACHE = 'pcw-pro-timesheet-v46'; // v46: Requests — Estimate/Appointment switch on the Schedule sheet (decides PO Log vs Estimate tab); an Estimate blocks a 30-min call, not the full job; quoted estimates stay listed so "they said yes" books the job and burns the PO; booked jobs email the customer a yes/no confirmation and show 🟠🟢🔴, and Move it re-asks them. Monthly estimate audit: AI vs booked vs clocked man-hours, with a calibration the ball-park reads; two-man crew default; entry number leads the customer name. v45: "Add time" — manual entries get the job/PO picker + materials, a Hours field that computes the end time, a date range for backdating several days, an Add-time button on the Clock screen, and an admin's own entries land approved. v44: scan a receipt from the Review screen and its lines fill Materials used; Take a photo vs From album; wide-lens toggle; an AI hiccup queues the photo instead of losing it. v43: clock-out time is the Submit tap; punches sent in the background (Background Sync + keepalive); waiting punches show their age, a day-old one is reported to the office
